...
export function updateTemplate(filepath, data) {
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      ].indexOf(templateFilename) === -1
    ) {
      return error(
        '[-f FILE] must be CONTRIBUTING.md, ISSUE_TEMPLATE.md or PULL_REQUEST_TEMPLATE.md (defaults to ISSUE_TEMPLATE.md)',
      );
    }
    ...
    fs.mkdir(path.dirname(filepath), () => {
      ...
      if (
        projectTemplate.indexOf(`https://opencollective.com/${data.slug}`) !==
        -1
      ) {
      ...
      if (projectTemplate && projectTemplate.length > 0) {
        ...
        const append_template_path = path.join(
          __dirname,
          `../templates/${append_template}`,
        );
