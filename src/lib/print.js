...
const collective_suggested_donation_amount =
  process.env.npm_package_collective_suggested_donation_amount;
const collective_suggested_donation_interval =
  process.env.npm_package_collective_suggested_donation_interval;
...
export function print(str, opts) {
  ...
  const terminalCols =
    ...
    (process.platform === 'win32'
      ? 80
      : parseInt(execSync('tput cols').toString()));
  const strLength = str.replace(/\u001b\[[0-9]{2}m/g, '').length;
  const leftPaddingLength =
    opts.align === 'center' ? Math.floor((terminalCols - strLength) / 2) : 2;
...
export function printStats(stats, opts) {
  ...
  print(
    `Annual budget: ${formatCurrency(stats.yearlyIncome, stats.currency)}`,
    opts,
  );
  print(
    `Current balance: ${formatCurrency(stats.balance, stats.currency)}`,
    Object.assign({}, { color: 'bold' }, opts),
  );
}
...
export function printFooter(collective) {
  console.log('');
  print(`Thanks for installing ${collective.slug} ${emoji('🙏')}`, {
    color: 'yellow',
  });
  ...
  console.log('');
  print(
    `${chalk.bold(`${emoji('👉 ')} Donate:`)} ${chalk.underline(
      getDonateURL(collective),
    )}`,
  );
  console.log('');
}
