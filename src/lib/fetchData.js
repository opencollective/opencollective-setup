...
const fetchStats = function(collectiveUrl) {
  ...
  return fetch(url, { timeout: 1500 })
    ...
    .catch(e => {
      const collectiveSlug = collectiveUrl.substr(
        collectiveUrl.lastIndexOf('/') + 1,
      );
      console.error(
        `${chalk.red(
          '[server error]',
        )} Cannot load the stats for ${collectiveSlug} – please try again later`,
      );
...
const fetchLogo = function(logoUrl) {
  ...
  return fetch(logoUrl, { timeout: 1500 })
    .then(res => {
      if (
        res.status === 200 &&
        res.headers.get('content-type').match(/^text\/plain/)
      )
        return res.text();
      else return '';
    })
