...
describe('updateTemplate.test.js', () => {
  after('rm CONTRIBUTING.md', done =>
    exec(`rm ${paths.newContributing}`, done),
  );
  after('clean', done => exec('npm run clean', done));

  it('creates a new CONTRIBUTING.md', () =>
    updateTemplate(paths.newContributing, { slug: 'testcollective' }).then(
      result => {
        ...
        expect(result.content).to.contain(
          'https://opencollective.com/testcollective/contributors.svg',
        );
      },
    ));

  it('appends to CONTRIBUTING.md', () =>
    updateTemplate(paths.contributing, { slug: 'testcollective' }).then(
      result => {
